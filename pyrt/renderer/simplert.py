...
class SimpleRT(Renderer):
    def __init__(self, shadow=False, iterations=1, background=(0,0,0)):
    ...
    def _shade(self, scene: Scene, ray: Ray, hitrecord: HitRecord) -> tuple:
        r,g,b = self.background  # background color
        hit = False
        ...
        if hit:
            element = hitrecord.obj
            # element hit -> call shader:
            material = hitrecord.material or element.material
            color = material.shade(scene.camera, ray, hitrecord, scene.lights)
            r = int(color[0] * 255)
            g = int(color[1] * 255)
            b = int(color[2] * 255)
